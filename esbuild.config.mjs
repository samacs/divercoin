...
async function buildAndReload() {
    ...
    const host = process.env.HOST || 'divercoin.local'
    const context = await esbuild.context({
        ...config,
        banner: {
            js: `(() => new EventSource("https://${host}:${port}").onmessage = () => location.reload())();`,
        },
    })
    ...
    let options = {}
    if (
        fs.existsSync('ssl/divercoin.local.pem') &&
        fs.existsSync('ssl/divercoin.local.key')
    ) {
        adapter = https
        options = {
            cert: fs.readFileSync('ssl/divercoin.local.pem'),
            key: fs.readFileSync('ssl/divercoin.local.key'),
        }
    }
